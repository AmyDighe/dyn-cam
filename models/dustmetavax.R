...
# It can then be run from a user-edited R script "dustmeta_run.R"
...
rate_infection_vaccinated[ , ] <- v_susc * rate_infection[i, j]
rate_infection_mAb_vaccinated[ , ] <- v_Ab_susc * rate_infection[i, j]
rate_reinfection_vaccinated[ , ] <- v_mAb_susc * rate_infection[i, j]
...
###############################################################
sigma_m <- user(4.42/360 ) # (/day) taken from catalytic modelling inference
v_sigma_m <- user(4.42/360)
...
## for vaccinated individuals
p_vSm[ , , ] <- 1 - exp(- (v_sigma_m + rate_infection_mAb[j, k] + mu[i] + rho)) 
p_vS[ , , ] <- 1 - exp(- (rate_infection[ j, k] + mu[i] + rho)) 
p_vI[] <- 1 - exp(- (v_gamma + mu[i] + rho)) 
p_vR[] <- 1 - exp(- (v_sigma + mu[i] + rho)) 
p_vS2[ , , ] <- 1 - exp(- (rate_reinfection[j, k] + mu[i] + rho)) 
...
new_I[25, nr -2, nc - 2] <- I[i, j, k] - outflow_I[i, j, k] + new_infections[i, j, k] + new_infections_mAb[i, j, k] + new_waned_vax_I[i, j, k] +
...
new_vax_Sm[ , , ] <- rbinom(new_Sm[i, j, k], vax[i])
new_vax_S[ , , ] <- rbinom(new_S[i, j, k], vax[i])
new_vax_I[ , , ] <- rbinom(new_I[i, j, k], vax[i])
new_vax_R[ , , ] <- rbinom(new_R[i, j, k], vax[i])
new_vax_S2[ , , ] <- rbinom(new_S2[i, j, k], vax[i])
new_vax_I2[ , , ] <- rbinom(new_I2[i, j, k], vax[i])

update(Sm[1, , ]) <- if(tt %% 30 == 0) 0 else new_Sm[1, j, k]
update(Sm[2:48, , ]) <- if(tt %% 30 == 0) new_Sm[i - 1, j, k] - new_vax_Sm[i - 1, j, k] else new_Sm[i, j, k]
update(Sm[N_age, , ]) <- if(tt %% 30 == 0) new_Sm[i - 1, j, k] - new_vax_Sm[i - 1, j, k] + new_Sm[i, j, k] - new_vax_Sm[i, j, k] else new_Sm[i, j, k]

update(S[1, , ]) <- if(tt %% 30 == 0) 0 else new_S[1, j, k]
update(S[2:48, , ]) <- if(tt %% 30 == 0) new_S[i - 1, j, k] - new_vax_S[i - 1, j, k] else new_S[i, j, k]
update(S[N_age, , ]) <- if(tt %% 30 == 0) new_S[i - 1, j, k] - new_vax_S[i - 1, j, k] + new_S[i, j, k] - new_vax_S[i, j, k] else new_S[i, j, k]

update(I[1 , , ]) <-  if(tt %% 30 == 0) 0 else new_I[1, j, k]
update(I[2:48, , ]) <- if(tt %% 30 == 0) new_I[i - 1, j, k] - new_vax_I[i - 1, j, k] else new_I[i, j, k]
update(I[25, , ]) <- if(tt %% 30 == 0) new_I[i - 1, j, k] - new_vax_I[i - 1, j, k] else if(tt == imp_t) 1 + new_I[i, j, k] else new_I[i, j, k]
update(I[N_age, , ]) <- if(tt %% 30 == 0) new_I[i - 1, j, k] - new_vax_I[i - 1, j, k] + new_I[i, j, k] - new_vax_I[i, j, k] else new_I[i, j, k]

update(R[1, , ]) <- if(tt %% 30 == 0) 0 else new_R[1, j, k]
update(R[2:48, , ]) <- if(tt %% 30 == 0) new_R[i - 1, j, k] - new_vax_R[i - 1, j, k] else new_R[i, j, k]
update(R[N_age, , ]) <- if(tt %% 30 == 0) new_R[i - 1, j, k] - new_vax_R[i - 1, j, k] + new_R[i, j, k] - new_vax_R[i, j, k] else new_R[i, j, k]

update(S2[1, , ]) <- if(tt %% 30 == 0) 0 else new_S2[1, j, k]
update(S2[2:48, , ]) <- if(tt %% 30 == 0) new_S2[i - 1, j, k] - new_vax_S2[i - 1, j, k] else new_S2[i, j, k]
update(S2[N_age, , ]) <- if(tt %% 30 == 0) new_S2[i - 1, j, k] - new_vax_S2[i - 1, j, k] + new_S2[i, j, k] - new_vax_S2[i, j, k] else new_S2[i, j, k]

update(I2[1, , ]) <- if(tt %% 30 == 0) 0 else new_I2[1, j, k]
update(I2[2:48, , ]) <- if(tt %% 30 == 0) new_I2[i - 1, j, k] - new_vax_I2[i - 1, j, k] else new_I2[i, j, k]
update(I2[N_age, , ]) <- if(tt %% 30 == 0) new_I2[i - 1, j, k] - new_vax_I2[i - 1, j, k] + new_I2[i, j, k] - new_vax_I2[i, j, k] else new_I2[i, j, k]

# and for vaccinated individuals
update(vSm[1, , ]) <- if(tt %% 30 == 0) 0 else new_vSm[1, j, k]
update(vSm[2:48, , ]) <- if(tt %% 30 == 0) new_vSm[i - 1, j, k] + new_vax_Sm[i - 1, j, k] else new_vSm[i, j, k]
update(vSm[N_age, , ]) <- if(tt %% 30 == 0) new_vSm[i - 1, j, k] + new_vax_Sm[i - 1, j, k] + new_vSm[i, j, k] + new_vax_Sm[i, j, k] else new_vSm[i, j, k]

update(vS[1, , ]) <- if(tt %% 30 == 0) 0 else new_vS[1, j, k]
update(vS[2:48, , ]) <- if(tt %% 30 == 0) new_vS[i - 1, j, k] + new_vax_S[i - 1, j, k] else new_vS[i, j, k]
update(vS[N_age, , ]) <- if(tt %% 30 == 0) new_vS[i - 1, j, k] + new_vax_S[i - 1, j, k] + new_vS[i, j, k] + new_vax_S[i, j, k] else new_vS[i, j, k]

update(vI[1 , , ]) <-  if(tt %% 30 == 0) 0 else new_vI[1, j, k]
update(vI[2:48, , ]) <- if(tt %% 30 == 0) new_vI[i - 1, j, k] + new_vax_I[i - 1, j, k] else new_vI[i, j, k]
update(vI[25, , ]) <- if(tt %% 30 == 0) new_vI[i - 1, j, k] + new_vax_I[i - 1, j, k] else if(tt == imp_t) 1 + new_vI[i, j, k] else new_vI[i, j, k]
update(vI[N_age, , ]) <- if(tt %% 30 == 0) new_vI[i - 1, j, k] + new_vax_I[i - 1, j, k] + new_vI[i, j, k] + new_vax_I[i, j, k] else new_vI[i, j, k]

update(vR[1, , ]) <- if(tt %% 30 == 0) 0 else new_vR[1, j, k]
update(vR[2:48, , ]) <- if(tt %% 30 == 0) new_vR[i - 1, j, k] + new_vax_R[i - 1, j, k] else new_vR[i, j, k]
update(vR[N_age, , ]) <- if(tt %% 30 == 0) new_vR[i - 1, j, k] + new_vax_R[i - 1, j, k] + new_vR[i, j, k] + new_vax_R[i, j, k] else new_vR[i, j, k]

update(vS2[1, , ]) <- if(tt %% 30 == 0) 0 else new_vS2[1, j, k]
update(vS2[2:48, , ]) <- if(tt %% 30 == 0) new_vS2[i - 1, j, k] + new_vax_S2[i - 1, j, k] else new_vS2[i, j, k]
update(vS2[N_age, , ]) <- if(tt %% 30 == 0) new_vS2[i - 1, j, k] + new_vax_S2[i - 1, j, k] + new_vS2[i, j, k] + new_vax_S2[i, j, k] else new_vS2[i, j, k]

update(vI2[1, , ]) <- if(tt %% 30 == 0) 0 else new_vI2[1, j, k]
update(vI2[2:48, , ]) <- if(tt %% 30 == 0) new_vI2[i - 1, j, k] + new_vax_I2[i - 1, j, k] else new_vI2[i, j, k]
update(vI2[N_age, , ]) <- if(tt %% 30 == 0) new_vI2[i - 1, j, k] + new_vax_I2[i - 1, j, k] + new_vI2[i, j, k] + new_vax_I2[i, j, k] else new_vI2[i, j, k]
...
dim(new_vax_Sm) <- c(N_age, nr, nc)
dim(new_vax_S) <- c(N_age, nr, nc)
dim(new_vax_I) <- c(N_age, nr, nc)
dim(new_vax_R) <- c(N_age, nr, nc)
dim(new_vax_S2) <- c(N_age, nr, nc)
dim(new_vax_I2) <- c(N_age, nr, nc)
