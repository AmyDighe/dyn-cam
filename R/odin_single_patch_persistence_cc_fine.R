...
estab <- vector(length = (dim(par_grid_cc_fine)[1]))
m_persist <- estab
persist <- estab
period_av <- estab
period_sd <- estab
...
for(i in 1:(dim(par_grid_cc_fine)[1])){
  
  imp_t <- par_grid_cc_fine$import_time[i] + (360 * seq(0,4, by = 1))
  e_t <- imp_t[5] + (3 * 360)
  m_t <- imp_t[5] + (25 * 360)
  p_t <- imp_t[5] + (50 * 360)
  
  x <- sir_model$new(alpha = alpha, beta = par_grid_cc_fine$beta[i], gamma = gamma, 
                     sigma = par_grid_cc_fine$waning[i], sigma_m = sigma_m, Ab_susc = Ab_susc, mAb_susc = mAb_susc, 
                     ...
                     N_0 = par_grid_cc_fine$pop[i], importation_rate = importation_rate, imp_t = imp_t, 
                     delta = par_grid_cc_fine$seasonality[i], ind1 = ind1, ind2 = ind2)
