...
Se <- matrix(nrow = 23, ncol = 3)
colnames(Se) <- scenario_core

beta_esti_lower_core <- beta_esti_lower[,core]
beta_esti_upper_core <- beta_esti_upper[,core]
beta_esti_core <- beta_esti[,core]
...
for(j in 1:3){
  ...
  for(i in 1:23){
    ...
    x <- sir_model$new(alpha = alpha, beta = beta, gamma = gamma, sigma = sigma,
                       ...
                       delta = delta, ind1 = ind1, ind2 = ind2, foi_bg_usr = foi_bg_usr)
...
for(j in 1:3){
  ...
  sigma <- par_grid_R0_core$sigma[j]
  reduced_shed <- par_grid_R0_core$red_shed[j]
  beta_vector <- beta_esti_upper_core[,j]
  mean_foi <- vector(length = length(beta_vector))
  ...
  saveRDS(file = "results/Se_upper.rds", object = Se)
...
for(j in 1:3){
  ...
  saveRDS(file = "results/Se.rds", object = Se)
